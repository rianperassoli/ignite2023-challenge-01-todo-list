...
interface TaskListProps {
  tasks: Task[];
  onClickDone: (task: Task) => void;
  onDeleteTask: (taskId: number) => void;
}

export function TaskList({ tasks, onClickDone, onDeleteTask }: TaskListProps) {
  return (
          ...
          <div className={styles.check}>
            <label className={styles.container}>
              <input
                type="checkbox"
                checked={task.done}
                onChange={() => onClickDone(task)}
              />
              <span className={styles.checkmark} />
            </label>
          </div>

          <p className={task.done ? styles.taskDone : styles.taskToDo}>
            {task.description}
          </p>

          <div className={styles.delete} onClick={() => onDeleteTask(task.id)}>
